...
- a214bdc: react: added a new sub-config `html` which uses [`@html-eslint/eslint-plugin-react`](https://npmx.dev/@html-eslint/eslint-plugin-react), ✅ enabled by default
...
- f84f5f9: vue: [Nuxt auto-imports](https://nuxt.com/docs/4.x/guide/concepts/auto-imports) are no longer reported as undefined by the `vue/nuxt` config, provided that `nuxt prepare` or `nuxt dev` has been run. This config also got a new option `buildDir` which points at the Nuxt build directory for when your Nuxt config cannot be loaded or lives above the directory ESLint runs in. Additionally, `v4DirectoryStructure` and `vueOrNuxtProjectDir` options now default to what your Nuxt config resolves to, instead of being guessed from the installed `nuxt` major version
- eeb11bd: [**BREAKING**] `plugin{Renames,Overrides}` root options have been replaced with `plugins.<pluginPrefix>.{prefix,plugin}`, where `plugins` is a new root option which accepts an object whose keys are plugin prefixes and values are objects to which the renaming and overriding functionality has moved. Additionally, all `settings` config options have been removed in favor of providing plugin settings via `plugins.<pluginPrefix>.settings`
- 9e89528: angular: updated [`@angular-eslint/{eslint-plugin,eslint-plugin-template,template-parser}` from v22.1.0 to v22.5.0](https://github.com/angular-eslint/angular-eslint/compare/v22.1.0...v22.5.0):
  
  - 🟢 enabled [`angular-template/prefer-style-binding`](https://github.com/angular-eslint/angular-eslint/blob/HEAD/packages/eslint-plugin-template/docs/rules/prefer-style-binding.md) rule and added it to the `noStylisticRules` config
  - 🟢 enabled [`angular/reactive-context-must-read-signal`](https://github.com/angular-eslint/angular-eslint/blob/HEAD/packages/eslint-plugin/docs/rules/reactive-context-must-read-signal.md) rule
- 113e50f: zod:
  
  - updated [`eslint-plugin-zod` from v4.9.0 to v4.13.1](https://github.com/marcalexiei/eslint-zod/compare/eslint-plugin-zod@4.9.0...eslint-plugin-zod@4.13.1):
  ...
  - updated [`eslint-plugin-zod-mini` from v1.6.0 to v1.10.1](https://github.com/marcalexiei/eslint-zod/compare/eslint-plugin-zod-mini@1.6.0...eslint-plugin-zod-mini@1.10.1):
    - 🟢 enabled the following rules:
  ...
  - updated [`eslint-plugin-zod-core` from v1.0.9 to v1.1.1](https://github.com/marcalexiei/eslint-zod/compare/eslint-plugin-zod-core@1.0.9...eslint-plugin-zod-core@1.1.1) and 🟢 enabled [`zod-core/prefer-validate`](https://github.com/marcalexiei/eslint-zod/blob/HEAD/plugins/eslint-plugin-zod-core/docs/rules/prefer-validate.md) rule (only if detected zod version is >=4.6)
- 3017073: un: added a new rule `un/no-distributive-never-check` that reports checking type parameters against `never`: `type IsNever<T> = T extends never ? true : false`.
  This is almost never what you want because conditional types are distributive with regard to a type parameter and are checking its union members one by one, but `never` is a union of zero members, which leads to the whole conditional resolving to the unexpected third value - neither `true` nor `false` - `never`.
- 6165804: `.gitignore` files are now being sought recursively by default (the `gitignore` root option now sets `recursive: true` by default)
- 25fcd22: ts: updated [`typescript-eslint` from v8.65.0 to v8.70.0](https://github.com/typescript-eslint/typescript-eslint/compare/v8.65.0...v8.70.0):
  
  - 🟢 enabled [`ts/no-generated-empty-object-type`](https://typescript-eslint.io/rules/no-generated-empty-object-type) rule
- a214bdc: angular: added a new sub-config `template/html` which uses [`@html-eslint/eslint-plugin-angular-template`](https://npmx.dev/@html-eslint/eslint-plugin-angular-template), ❓ enabled if Angular version is at least 18 and `template` sub-config is not disabled
- 6766b0a: playwright: updated [`eslint-plugin-playwright` from v2.11.0 to v2.12.0](https://github.com/mskelton/eslint-plugin-playwright/compare/v2.11.0...v2.12.0):
  
  - 🟢 enabled [`playwright/no-action-timeout`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-action-timeout.md) rule
  - 🟢 enabled [`playwright/no-export`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-export.md) rule
  - 🟢 enabled [`playwright/no-identical-title`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-identical-title.md) rule
  - 🟡 enabled [`playwright/no-magic-timeouts`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-magic-timeouts.md) rule (warning)
  - 🟢 enabled [`playwright/no-template-literal-title`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-template-literal-title.md) rule
  - 🟢 enabled [`playwright/no-test-return-statement`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/no-test-return-statement.md) rule
  - 🔴 not enabled [`playwright/prefer-ending-with-an-expect`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/prefer-ending-with-an-expect.md) rule
  - 🟢 enabled [`playwright/require-annotation-reason`](https://github.com/mskelton/eslint-plugin-playwright/blob/HEAD/docs/rules/require-annotation-reason.md) rule
- ae2326c: solid: updated [`eslint-plugin-solid` from v0.14.5 to v0.18.0](https://github.com/solidjs-community/eslint-plugin-solid/compare/v0.14.5...v0.18.0):
...
- a214bdc: svelte: added a new sub-config `html` which uses [`@html-eslint/eslint-plugin-svelte`](https://npmx.dev/@html-eslint/eslint-plugin-svelte), ✅ enabled by default
...
- a171679: Added a new config `safeql` which uses [`@ts-safeql/eslint-plugin`](https://npmx.dev/@ts-safeql/eslint-plugin), ❓ enabled if `@ts-safeql/sql-tag` package is installed
- 5a48cdb: [**BREAKING**] betterTailwind: removed the `cssLinting` option in favor of the new `configCss` sub-config, which lints CSS files in a dedicated ESLint config instead of appending the `files` of the `css` config to the parent ones. Specifying `files` of the parent config now disables it unless `configCss` is set explicitly.
- 48c0307: [**BREAKING**] import: added a new option `extraneousDependenciesCheck` that replaces `allowDevDependencies` and `extraneousDependenciesWhitelist` and provides better control of the [`import/no-extraneous-dependencies`](https://github.com/un-ts/eslint-plugin-import-x/blob/HEAD/docs/rules/no-extraneous-dependencies.md) rule options, including the new `packageDir` sub-option that specifies the directories to read `package.json` files from - useful in monorepos, where the root `package.json` is otherwise invisible to the files inside workspace packages. Additionally, when `mode` root option is set to `lib`, all `files` from the `tests` config combined with the patterns targeting config files will not be considered library code and therefore allowed to import modules listed in `devDependencies`; this stops flagging `vitest` imports, for example
- 038f5ef: vue: added a new `enforcePropsDestructuring` option to configure [`vue/define-props-destructuring`](https://eslint.vuejs.org/rules/define-props-destructuring.html) rule, which forbids destructuring by default
- c6929a6: `gitignore` root option no longer requires a `.gitignore` file to be present in the current working directory: a nested file, a file in a parent directory or `.gitmodules` are now each enough on their own (`strict: false` is now passed to the underlying package by default). Additionally, the resolved configs cache is now keyed on the generated ignore patterns instead of the root `.gitignore` contents only, so editing a nested file no longer serves a stale config
- 948f984: Added a new root option `baselineAvailability` which sets the `available` option of every `use-baseline` rule: [`css/use-baseline`](https://github.com/eslint/css/blob/HEAD/docs/rules/use-baseline.md), [`html/use-baseline`](https://html-eslint.org/docs/rules/use-baseline), [`html-angular/use-baseline`](https://html-eslint.org/docs/angular-template/rules/use-baseline), [`html-react/use-baseline`](https://html-eslint.org/docs/react/rules/use-baseline) and [`html-svelte/use-baseline`](https://html-eslint.org/docs/svelte/rules/use-baseline)
- a02b817: sonar: updated [`eslint-plugin-sonarjs` from v4.2.0 to v4.2.1](https://github.com/SonarSource/SonarJS/blob/6b11c6ee96a415548f9e754f6764a0b3c30aa45f/packages/analysis/src/jsts/rules/CHANGELOG.md#2026-09-15-version-421):
...
- 738153a: vue: updated [`eslint-plugin-vue` from v10.10.0 to v10.11.0](https://github.com/vuejs/eslint-plugin-vue/compare/v10.10.0...v10.11.0):
  
  - 🟢 enabled [`vue/no-shadow-native-events`](https://eslint.vuejs.org/rules/no-shadow-native-events.html) rule
- e03f2dd: Now, when a required dependency fails to resolve, the reason for why it was requested is shown (for example, "used in config X" for plugins)
...
- 10a68b0: import: added a new sub-config `allowDefaultExport`, enabled by default, which gives explicit control over which files are allowed to have default exports
- 4315067: [**BREAKING**] Dropped support for ESLint 9 as [it reached end of life on 2026-08-06](https://eslint.org/version-support).
...
- 508c98d: [**BREAKING**] Dropped support for Node.js 25, which has reached its end-of-life, and raised the minimum supported v24 and v26 versions. The officially supported Node.js versions range is now `^22.23.1 || ^24.18.0 || >=26.4.0`
...
- a1961db: nestJs: updated [`@darraghor/eslint-plugin-nestjs-typed` from v7.3.0 to v7.5.3](https://github.com/darraghoriordan/eslint-plugin-nestjs-typed/compare/v7.3.0...v7.5.3):
...
- 59dbddb: nodeDependencies: fixed `enforceAbsoluteVersion: true` not enforcing absolute versions on `dependencies`, contrary to its documentation. Also added an `'always'` value, which additionally covers `peerDependencies` and `optionalDependencies`
...
- d68e151: jsdoc: updated [`eslint-plugin-jsdoc` from v63.3.3 to v64.4.0](https://github.com/gajus/eslint-plugin-jsdoc/compare/v63.3.3...v64.4.0):
  ...
  - 🔴 not enabled [`jsdoc/ts-ban-ts-comment`](https://github.com/gajus/eslint-plugin-jsdoc/blob/HEAD/docs/rules/ts-ban-ts-comment.md) rule
- 2e49917: cli: removed the `disabledRules` option in favor of making the `overrides` option strictly typed
- 1e2d81d: unicorn: added `unicorn/inline-js` eslint config, created when the `jsInline` config is enabled, which disables [`unicorn/no-empty-file`](https://github.com/sindresorhus/eslint-plugin-unicorn/blob/HEAD/docs/rules/no-empty-file.md) on HTML files: every `<script>` block of such a file is linted as a separate file, so an empty one was reported as an empty file

### Patch Changes

- 7676a39: ts: [`no-class-assign`](https://eslint.org/docs/latest/rules/no-class-assign) and [`no-with`](https://eslint.org/docs/latest/rules/no-with) are now disabled in TypeScript files because the TypeScript compiler already reports these errors
...
- 1606425: `typeInfoRules.allowDefaultProject` and `typeInfoRules.parserOptions` are now merged instead of being mutually exclusive. Previously, specifying both silently discarded `parserOptions`
...
- f834df6: awsCdk: updated [`eslint-plugin-awscdk` from v4.4.0 to v4.4.3](https://github.com/ren-yamanashi/awscdk-lint/compare/eslint-plugin-awscdk@4.4.0...eslint-plugin-awscdk@4.4.3)
...
- 91e5f1f: eslintComments: updated [`@eslint-community/eslint-plugin-eslint-comments` from v4.7.2 to v4.8.1](https://github.com/eslint-community/eslint-plugin-eslint-comments/compare/v4.7.2...v4.8.1)
...
- e2b69ce: nextJs: updated [`@next/eslint-plugin-next` from v16.3.0 to v16.3.5](https://github.com/vercel/next.js/compare/v16.3.0...v16.3.5)
- 73e8b92: nx: updated [`@nx/eslint-plugin` from v23.1.1 to v23.2.1](https://github.com/nrwl/nx/compare/23.1.1...23.2.1)
- 4d4d8c2: unocss: updated [`@unocss/eslint-plugin` from v66.7.5 to v66.10.3](https://github.com/unocss/unocss/compare/v66.7.5...v66.10.3)
...
- 80f7159: html: updated [`@html-eslint/{eslint-plugin,parser}` from v0.64.0 to v0.66.0](https://github.com/yeonjuan/html-eslint/compare/v0.64.0...v0.66.0)
- eae5329: jest: updated [`eslint-plugin-jest` from v29.16.0 to v29.16.6](https://github.com/jest-community/eslint-plugin-jest/compare/v29.16.0...v29.16.6)
- 337f865: tsrx: updated [`@tsrx/eslint-{plugin,parser}` from v0.3.118 to v0.3.124](https://github.com/Ripple-TS/ripple/compare/%40tsrx/eslint-plugin%400.3.118...%40tsrx/eslint-plugin%400.3.124) and [from v0.3.124 to v0.4.1](https://github.com/tsrx-org/tsrx/compare/%40tsrx/eslint-plugin%400.3.126...%40tsrx/eslint-plugin%400.4.1):
  
  - ❌ The following rules were removed:
    - `tsrx/no-module-scope-track`
    - `tsrx/prefer-oninput`
    - `tsrx/no-lazy-destructuring-in-modules`
...
- 9a3d030: [**BREAKING**] sonar: changed the default `eslint-plugin-sonarjs` prefix from `sonarjs` to `sonar`
...
- 4a976ed: turbo: updated [`eslint-plugin-turbo` from v2.10.8 to v2.10.13](https://github.com/vercel/turborepo/compare/v2.10.8...v2.10.13)
- c8fb1d6: Fixed an issue where the progress indicator drawn by the `fileProgress` config was not shown on some implicitly ignored file extensions: they are no longer ignored
...
- 943b1db: js, ts: core rules misfiring on TypeScript syntax or duplicating the TypeScript compiler, like [`no-unused-vars`](https://eslint.org/docs/latest/rules/no-unused-vars) and [`no-undef`](https://eslint.org/docs/latest/rules/no-undef), are now disabled in TypeScript files even when the `ts` config doesn't lint them. Base rules of type-aware extension rules, like [`require-await`](https://eslint.org/docs/latest/rules/require-await), are now disabled only where their `ts/*` replacements are enabled
...
- b2acfee: [**BREAKING**] sonar: moved to `misc-enabled` configs and [`eslint-plugin-sonarjs`](https://npmx.dev/eslint-plugin-sonarjs) to optional peer dependencies, because it depends on `typescript`, which was enabling the `ts` config in projects not having it installed
- 0d66422: security: disabled `security/detect-unsafe-regex` because of a high number of false positives. We recommend using rules from the [`eslint-plugin-regexp` plugin](https://ota-meshi.github.io/eslint-plugin-regexp) instead (the `regexp` config)
...
- 05d9bf9: formatJs: updated [`eslint-plugin-formatjs` from v6.4.20 to v7.0.0](https://github.com/formatjs/formatjs/compare/eslint-plugin-formatjs@6.4.20...eslint-plugin-formatjs@7.0.0):
  
  - 🔴 not enabled [`formatjs/enforce-message-types`](https://formatjs.github.io/docs/tooling/linter/#enforce-message-types) rule
...
- db4997c: noPrettierIncompatibleRules: stopped disabling the following rules when the Config is on because most of the time they don't conflict with Prettier formatting:
...
- d19959e: packageJson: updated [`eslint-plugin-package-json` from v1.6.3 to v1.8.1](https://github.com/michaelfaith/eslint-plugin-package-json/compare/v1.6.3...v1.8.1):
...
- 2f827de: ngrx: updated [`@ngrx/eslint-plugin` from v21.1.1 to v22.0.1](https://github.com/ngrx/platform/compare/21.1.1...22.0.1)
...
- a61f2d6: Fixed an incorrect mapping of the graphql's `requireSeparateFilesFor` option to [`graphql/lone-executable-definition`](https://the-guild.dev/graphql/eslint/rules/lone-executable-definition) rule `ignore` option. The option docs now also warn that all 4 values cannot be set to `false`
- c86d196: cspell: updated [`@cspell/eslint-plugin` from v10.0.1 to v10.3.2](https://github.com/streetsidesoftware/cspell/compare/v10.0.1...v10.3.2)
- ab6ae70: unhead: updated [`@unhead/eslint-plugin` from v3.3.1 to v3.4.1](https://github.com/unjs/unhead/compare/v3.3.1...v3.4.1)
- 2fb73f3: cypress: updated [`eslint-plugin-cypress` from v6.4.3 to v7.0.2](https://github.com/cypress-io/eslint-plugin-cypress/compare/v6.4.3...v7.0.2)
- 84288a0: vitest: updated [`@vitest/eslint-plugin` from v1.6.26 to v1.6.27](https://github.com/vitest-dev/eslint-plugin-vitest/compare/v1.6.26...v1.6.27)
...
- a71837b: react: updated [`@eslint-react/eslint-plugin` and `eslint-plugin-react-debug` from v5.18.3 to v5.19.1](https://github.com/Rel1cx/eslint-react/compare/v5.18.3...v5.19.1)
- 9a1c48e: regexp: updated [`eslint-plugin-regexp` from v3.1.1 to v3.3.0](https://github.com/ota-meshi/eslint-plugin-regexp/compare/v3.1.1...v3.3.0)
...
- eb138a1: react: updated [`eslint-plugin-react-refresh` from v0.5.3 to v0.5.7](https://github.com/ArnaudBarre/eslint-plugin-react-refresh/compare/v0.5.3...v0.5.7)
- e3fd349: [**BREAKING**] yaml: removed `doNotMergeIgnoresWithDefault` in favor of the function form of `ignores`, which replaces the default ignore list: `ignores: () => ['**/my-glob']`. Also removed `ignoresAdditional`, which never had any effect
