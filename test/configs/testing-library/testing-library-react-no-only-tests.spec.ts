...
describe('testing-library: sub config `react.noOnlyTests`', () => {
  describe('basic tests', () => {
    it('creates `testing-library/react/no-only-tests` eslint config by default', async () => {
      const configResult = await computeEslintConfig({testingLibrary: {configReact: true}});

      const config = configResult.getConfigByUnPostfix('testing-library/react/no-only-tests');
