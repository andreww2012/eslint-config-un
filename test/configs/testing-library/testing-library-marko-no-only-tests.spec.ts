...
describe('testing-library: sub config `marko.noOnlyTests`', () => {
  describe('basic tests', async () => {
    ...
    it('does not create `testing-library/marko/no-only-tests` eslint config when the parent `testing-library/marko` eslint config is disabled via empty `files`', async () => {
      const configResult = await computeEslintConfig({testingLibrary: {configMarko: {files: []}}});
  ...
  describe('rules', async () => {
    ...
    it('enables `no-only-tests/no-only-tests` rule by default', () => {
      expect(
        configResult.getRuleEntrySeverity(
          'testing-library/marko/no-only-tests',
    ...
    it('`no-only-tests/no-only-tests` rule fires on a test with the `.only` modifier', async () => {
      const results = await testEslintConfig(
        {testingLibrary: {configMarko: true}},
  ...
  describe('un options', () => {
    describe('option: `files`', () => {
      it('uses user-provided `files` in `testing-library/marko/no-only-tests` eslint config', async () => {
        const FILES = ['tests/**/*.spec.ts'];

        const configResult = await computeEslintConfig({
          testingLibrary: {configMarko: {configNoOnlyTests: {files: FILES}}},
        });
      ...
      it('disables `testing-library/marko/no-only-tests` eslint config when set to empty array', async () => {
        const configResult = await computeEslintConfig({
          testingLibrary: {configMarko: {configNoOnlyTests: {files: []}}},
        });
    ...
    describe('option: `ignores`', () => {
      it('uses user-provided `ignores` in `testing-library/marko/no-only-tests` eslint config and merges them with defaults', async () => {
        const IGNORES = ['**/fixtures/**'];

        const configResult = await computeEslintConfig({
          testingLibrary: {configMarko: {configNoOnlyTests: {ignores: IGNORES}}},
        });
    ...
    it('respects `overrides` and `overridesAny` in `testing-library/marko/no-only-tests` eslint config', async () => {
      const configResult = await computeEslintConfig({
        testingLibrary: {
          configMarko: {
            configNoOnlyTests: {
      ...
      expect(configResult.getRuleSeverities('testing-library/marko/no-only-tests')).toMatchObject({
