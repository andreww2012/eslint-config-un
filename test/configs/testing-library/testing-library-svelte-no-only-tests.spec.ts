...
describe('testing-library: sub config `svelte.noOnlyTests`', () => {
  describe('basic tests', async () => {
    ...
    it('does not create `testing-library/svelte/no-only-tests` eslint config when the parent `testing-library/svelte` eslint config is disabled via empty `files`', async () => {
      const configResult = await computeEslintConfig({testingLibrary: {configSvelte: {files: []}}});
  ...
  describe('rules', async () => {
    ...
    it('correctly sets severities by default', () => {
      expect(configResult.getRuleSeverities('testing-library/svelte/no-only-tests')).toMatchObject({
    ...
    it('`no-only-tests/no-only-tests` rule fires on a test with the `.only` modifier', async () => {
      const results = await testEslintConfig(
        {testingLibrary: {configSvelte: true}},
  ...
  describe('un options', () => {
    describe('option: `files`', () => {
      it('uses user-provided `files` in `testing-library/svelte/no-only-tests` eslint config', async () => {
        const FILES = ['tests/**/*.spec.ts'];

        const configResult = await computeEslintConfig({
          testingLibrary: {configSvelte: {configNoOnlyTests: {files: FILES}}},
        });
      ...
      it('disables `testing-library/svelte/no-only-tests` eslint config when set to empty array', async () => {
        const configResult = await computeEslintConfig({
          testingLibrary: {configSvelte: {configNoOnlyTests: {files: []}}},
        });
    ...
    describe('option: `ignores`', () => {
      it('uses user-provided `ignores` in `testing-library/svelte/no-only-tests` eslint config and merges them with defaults', async () => {
        const IGNORES = ['**/fixtures/**'];

        const configResult = await computeEslintConfig({
          testingLibrary: {configSvelte: {configNoOnlyTests: {ignores: IGNORES}}},
        });

        const ignores = configResult.getConfigByUnPostfix(
          'testing-library/svelte/no-only-tests',
        )?.ignores;
    ...
    it('respects `overrides` and `overridesAny` in `testing-library/svelte/no-only-tests` eslint config', async () => {
      const configResult = await computeEslintConfig({
        testingLibrary: {
          configSvelte: {
            configNoOnlyTests: {
