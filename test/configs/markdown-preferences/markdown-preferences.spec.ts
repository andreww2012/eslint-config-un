...
describe('basic tests', async () => {
  ...
  describe('mode: all configs are disabled', () => {
    it('does not create `markdown-preferences` eslint config', async () => {
      await expectConfigState({}, 'markdown-preferences', false);
    });

    it('creates `markdown-preferences` eslint config if explicitly enabled', async () => {
      await expectConfigState('markdownPreferences', 'markdown-preferences', true);
    });
  });

  describe('mode: all configs are not explicitly enabled or disabled', () => {
    it('creates `markdown-preferences` eslint config', async () => {
      await expectConfigState({}, 'markdown-preferences', true, 'default');
    });
  ...
  describe('mode: misc configs are enabled', () => {
    it('creates `markdown-preferences` eslint config', async () => {
      await expectConfigState({}, 'markdown-preferences', true, 'misc-enabled');
    });
...
describe('options', () => {
  ...
  describe('option: `casingEnforcementIgnorePatterns`', () => {
    it('merges array-provided patterns with default ignore patterns for heading-casing and table-header-casing rules', async () => {
      ...
      expect(
        ...
        severity: 2,
        options: [{ignorePatterns: expect.arrayContaining(PATTERNS) as unknown}],
      });
    ...
    it('merges object-provided patterns with default ignore patterns, excluding falsy-valued entries', async () => {
      ...
      const REMOVED_PATTERN = String.raw`/^v\d+/u`;
      ...
      expect(headingCasingRuleEntry).toMatchObject({
        severity: 2,
        options: [{ignorePatterns: expect.arrayContaining([ADDED_PATTERN]) as unknown}],
      });
      expect(headingCasingRuleEntry.options).toMatchObject([
        {ignorePatterns: expect.not.arrayContaining([REMOVED_PATTERN]) as unknown},
      ]);
  ...
  describe('option: `orderedLists`', () => {
    ...
    it('disables `markdown-preferences/ordered-list-marker-style` rule when `style` is `false`', async () => {
      const configResult = await computeEslintConfig({
    ...
    it('directly passes options passed to `style` to `markdown-preferences/ordered-list-marker-style` rule', async () => {
      const OPTIONS = {
        overrides: [{level: 2, parentMarker: 'bullet' as const}],
        prefer: 'n)' as const,
      };

      const configResult = await computeEslintConfig({
        markdownPreferences: {orderedLists: {style: OPTIONS}},
      });
  ...
  describe('option: `wordsToPreserveCasingOf`', () => {
    it('merges array-provided words with plugin default preserve-words list', async () => {
      ...
      expect(
        ...
        severity: 2,
        options: [{preserveWords: expect.arrayContaining(WORDS) as unknown}],
      });
      expect(
        configResult.getRuleEntryParsed(
          'markdown-preferences',
    ...
    it('merges object-provided words with plugin default preserve-words list, excluding falsy-valued entries', async () => {
      ...
      expect(headingCasingRuleEntry).toMatchObject({
        severity: 2,
        options: [{preserveWords: expect.arrayContaining([ADDED_WORD]) as unknown}],
      });
      expect(headingCasingRuleEntry.options).toMatchObject([
        {preserveWords: expect.not.arrayContaining([REMOVED_WORD]) as unknown},
      ]);
