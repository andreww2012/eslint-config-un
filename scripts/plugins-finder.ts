// TODO npm-stats gets rate limited and returns empty stats
...
async function fetchPackageMetadata(packageName: string) {
  ...
  try {
    ...
    if (error instanceof PackageNotFoundError) {
      ...
      logger.error(
        `Error fetching package ${styleText('blueBright', packageName)} metadata:`,
        error,
      );
      if (
...
const fetchPackageStats = pThrottle({limit: 1, interval: 700, strict: true})(async (
  packageName: string,
) => {
...
async function analyze() {
  ...
  const pluginsAnalyzed = pluginsInfo
    .map(({key, value: info}) => {
      ...
      const downloadsPerDayAverage = totalDownloads / downloadsPerDay.length;
      ...
      if (['eslint-plugin-emotion', 'eslint-plugin-es'].includes(name)) {
        console.log({
          name,
          totalDownloads,
          scoreRecency,
          scoreDownloads,
          scoreDependencies,
          scoreEslintPluginsDependencies,
        });
      }
